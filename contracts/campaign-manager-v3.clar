    ...
    (map-set campaigns
      {id: campaign-id}
      {
        ...
        created-at: burn-block-height
            ...
            (unwrap-panic (contract-call? .distribution-engine-v4 initialize-campaign-pool campaign-id))
...
(define-public (donate-to-disaster-relief
  ...
  (let (
    ...
    (try! (contract-call? .donation-targeting-v4 make-targeted-donation campaign-id amount target-org-id))
    
    ;; Deposit to distribution pool
    (try! (contract-call? .distribution-engine-v4 deposit-to-campaign campaign-id amount tx-sender))
    
    ;; Issue receipt NFT to donor
    (try! (contract-call? .nft-receipts-v4 mint-receipt 
            tx-sender 
            campaign-id 
            "donation" 
            amount 
...
(define-public (setup-disaster-relief-beneficiaries
  ...
  (let (
    ...
    (let (
      ...
      (try! (contract-call? .distribution-engine-v4 batch-set-allocations campaign-id recipients-shares))
...
(define-private (issue-beneficiary-receipt
  ...
  (match prev-result
    success (contract-call? .nft-receipts-v4 mint-receipt 
...
(define-public (create-company-payroll-campaign
  ...
  (let (
    (admin tx-sender)
    (company-data (unwrap! (contract-call? .company-auth-v4 get-company-by-admin admin) ERR_UNAUTHORIZED))
  )
    ;; Create campaign
    (let (
...
(define-public (setup-payroll-campaign
  ...
  (let (
    ...
    (let (
      ...
      (try! (contract-call? .hierarchy-calculator-v4 batch-calculate-payroll campaign-id employee-list))
...
(define-private (setup-employee-fold 
  ...
  (match prev-result
    success (match (contract-call? .hierarchy-calculator-v4 assign-employee
...
(define-private (set-employee-allocation-fold 
  ...
  (match prev-result
    success (let (
              ...
              (calculation (contract-call? .hierarchy-calculator-v4 get-payroll-calculation campaign-id employee))
            )
            (match calculation
              calc-result (match (contract-call? .distribution-engine-v4 set-allocation 
                                   campaign-id 
                                   employee 
...
(define-public (process-payroll
  ...
  (let (
    ...
    (try! (contract-call? .distribution-engine-v4 deposit-to-campaign campaign-id total-amount tx-sender))
...
(define-private (issue-salary-receipt-fold 
  ...
  (match prev-result
    success (let (
            ...
            (match calculation
              calc-result (match (contract-call? .nft-receipts-v4 mint-receipt 
                                   employee
                                   campaign-id
                                   "salary"
                                   (get final-shares calc-result)
...
(define-public (claim-disaster-relief-funds (campaign-id uint))
  (let (
    (campaign (unwrap! (get-campaign campaign-id) ERR_NOT_FOUND))
    (pending-amount (contract-call? .distribution-engine-v4 get-pending-rewards campaign-id tx-sender))
  )
    (asserts! (is-eq (get campaign-type campaign) CAMPAIGN_TYPE_DISASTER_RELIEF) ERR_INVALID_AMOUNT)
    (asserts! (> pending-amount u0) ERR_INVALID_AMOUNT)
    
    ;; Claim from distribution engine
    (try! (contract-call? .distribution-engine-v4 claim-rewards campaign-id))
...
(define-read-only (get-campaign-transparency (campaign-id uint))
  (contract-call? .donation-targeting-v4 get-campaign-donations campaign-id)
)

;; Get user donation history
(define-read-only (get-user-donations (campaign-id uint) (user principal))
  (contract-call? .donation-targeting-v4 get-donor-transparency campaign-id user)
)

;; Get company employees (for transparency)
(define-read-only (get-company-employees-for-campaign (admin principal))
  (contract-call? .company-auth-v4 get-company-employees u1)
)